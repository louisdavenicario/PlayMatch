...
export const supabase = createClient(supabaseUrl, supabaseAnonKey, {
  auth: {
    persistSession: true,       // Ensures the login survives when Chrome is closed
    autoRefreshToken: true,     // Automatically refreshes the "key" so they aren't logged out while active
    detectSessionInUrl: true,   // Helps maintain the session state across different tabs
    storageKey: 'sb-auth-token' // Standard key name for local storage
  }
});